const servicesLeft = ['Routine Blood Draws','Specialty Lab Collections','Standing Orders','Wellness Testing'];
const servicesRight = ['Employer & Occupational Testing','Assisted Living & Facility Visits','Pediatric Collections','Hospital Discharge Collections'];
const steps = [
    ...
    <div className="page">
      <header className="header">
        <div className="wrap header-inner">
          <a className="logo" href="#top"><span className="logo-mark">♡</span><span><strong>MRS</strong><small>MEDICAL SERVICES</small></span></a>
          <nav>
            <a href="#services">SERVICES</a><a href="#process">HOW IT WORKS</a><a href="#area">SERVICE AREA</a><a href="#about">ABOUT</a><a href="#faq">FAQS</a><a href="#contact">CONTACT</a>
          </nav>
        </div>
      </header>
      ...
      <main id="top">
        <section className="hero">
          <div className="hero-copy">
            ...
            <div className="hero-buttons"><a className="btn primary" href="#contact">SCHEDULE A VISIT</a><a className="btn secondary" href="tel:+17327660266">⌕&nbsp;&nbsp; CALL NOW</a></div>
            <div className="zip-box" id="area">
              <div className="pin">⌖</div>
              <div><strong>Need a blood draw?</strong><span>Enter your ZIP code to confirm we service your area.</span>
                <div className="zip-row"><input placeholder="Enter ZIP code" inputMode="numeric"/><button>CHECK AVAILABILITY</button></div>
              </div>
            </div>
          </div>
        ...
        <section className="services" id="services"><div className="wrap services-grid">
          <div className="section-title"><span>SERVICES</span><h2>Blood Collection<br/>Made Easy</h2><i></i><p>We provide a full range of mobile phlebotomy services for individuals, families, providers, and organizations.</p></div>
          <div className="service-column">{servicesLeft.map(x=><div key={x}><Check/>{x}</div>)}</div>
          <div className="service-column">{servicesRight.map(x=><div key={x}><Check/>{x}</div>)}</div>
        </div></section>

        <section className="process" id="process"><div className="wrap process-grid">
          <div className="section-title"><span>HOW IT WORKS</span><h2>Simple. Safe.<br/>Convenient.</h2><i></i></div>
          <div className="steps">{steps.map(([n,t,d],i)=><article key={n}><div className="step-top"><b>{n}</b>{i<3&&<em>→</em>}</div><strong>{t}</strong><p>{d}</p></article>)}</div>
        </div></section>
